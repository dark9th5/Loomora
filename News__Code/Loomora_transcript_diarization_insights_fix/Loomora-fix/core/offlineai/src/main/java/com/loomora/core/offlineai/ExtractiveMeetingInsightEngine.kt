...
class HeuristicMeetingInsightEngine @Inject constructor(
    ...
    override suspend fun analyze(input: MeetingInsightInput): MeetingInsightOutput = withContext(Dispatchers.Default) {
        ...
        val actionCandidates = selectDiverse(scored.filter { actionRegex.containsMatchIn(it.text) }, 3)
        ...
        val actions = actionCandidates.map { candidate ->
            ActionItem(
                task = cleanSentence(candidate.text),
                assignee = explicitAssignee(candidate.text),
                dueDate = explicitDueDate(candidate.text),
                evidenceSegmentIds = candidate.segmentIds
            )
    ...
    private fun explicitAssignee(text: String): String? =
        Regex("\\b(?:ask|asks|assigned to|giao cho|nhờ)\\s+([\\p{L}][\\p{L}\\p{N}_-]{1,24})", RegexOption.IGNORE_CASE)
            .find(text)?.groupValues?.getOrNull(1)

    private fun explicitDueDate(text: String): String? =
        Regex("\\b(?:today|tomorrow|friday|monday|tuesday|wednesday|thursday|saturday|sunday|hôm nay|ngày mai|thứ hai|thứ ba|thứ tư|thứ năm|thứ sáu|thứ bảy|chủ nhật)\\b", RegexOption.IGNORE_CASE)
            .find(text)?.value
