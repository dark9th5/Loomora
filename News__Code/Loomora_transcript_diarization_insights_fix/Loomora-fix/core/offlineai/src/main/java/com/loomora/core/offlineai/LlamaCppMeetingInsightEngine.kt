...
class LlamaCppMeetingInsightEngine @Inject constructor(
    ...
    override suspend fun analyze(input: MeetingInsightInput): MeetingInsightOutput = withContext(Dispatchers.IO) {
        ...
        val validSegmentIds = analysisSegments.map { it.id }.filter(String::isNotBlank).toSet()
    ...
    private fun buildPrompt(chunks: List<TranscriptChunk>): String {
        val transcript = chunks.joinToString("\n") { chunk ->
            chunk.segments.joinToString("\n") { segment ->
                "[${segment.id} ${segment.startMs}-${segment.endMs} ${segment.speakerLabel ?: "Unknown speaker"}] ${segment.text}"
            }
        }
        return """
            Analyze the complete multi-speaker conversation and return meeting insights as JSON only.
            First combine adjacent fragments from the same speaker mentally. Synthesize overall meaning instead of copying short phrases.
            The summary must explain the main topic, important discussion points, conclusions, and next steps in 2-5 sentences.
            Return diverse, deduplicated key points. Use only listed evidence segment IDs.
            Keep assignee and dueDate null unless explicit in the transcript. Separate decisions, suggestions, and open questions.
