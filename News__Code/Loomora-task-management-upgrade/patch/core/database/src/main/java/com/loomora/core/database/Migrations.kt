        ...
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL(
                ...
                CREATE TABLE IF NOT EXISTS `recording_tasks` (
                    `id` TEXT NOT NULL,
                    `recordingId` TEXT NOT NULL,
                    `sourceInsightRevisionId` TEXT,
                    `sourceActionIndex` INTEGER,
                    `title` TEXT NOT NULL,
                    `assignee` TEXT,
                    `dueDate` TEXT,
                    `status` TEXT NOT NULL DEFAULT 'TODO',
                    `evidenceSegmentIdsCsv` TEXT NOT NULL DEFAULT '',
                    `sourceGenerationMode` TEXT NOT NULL DEFAULT 'UNKNOWN',
                    `createdAt` INTEGER NOT NULL,
                    `updatedAt` INTEGER NOT NULL,
                    `completedAt` INTEGER,
            ...
            database.execSQL("CREATE INDEX IF NOT EXISTS `index_recording_tasks_recordingId` ON `recording_tasks` (`recordingId`)")
            database.execSQL("CREATE INDEX IF NOT EXISTS `index_recording_tasks_status` ON `recording_tasks` (`status`)")
            database.execSQL("CREATE INDEX IF NOT EXISTS `index_recording_tasks_sourceInsightRevisionId` ON `recording_tasks` (`sourceInsightRevisionId`)")
        }
    }
}
