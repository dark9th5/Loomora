...
internal fun EditTaskDialog(
    task: RecordingTaskEntity,
    onConfirm: (String, String?, String?) -> Unit,
    onDismiss: () -> Unit
) {
    var title by remember(task.id, task.title) { mutableStateOf(task.title) }
    var assignee by remember(task.id, task.assignee) { mutableStateOf(task.assignee.orEmpty()) }
    var dueDate by remember(task.id, task.dueDate) { mutableStateOf(task.dueDate.orEmpty()) }
    val vietnamese = Locale.getDefault().language.equals("vi", ignoreCase = true)
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(if (vietnamese) "Chỉnh sửa việc cần làm" else "Edit action item") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(
                    value = title,
                    onValueChange = { title = it },
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text(if (vietnamese) "Nội dung công việc" else "Task") },
                    minLines = 2,
                    maxLines = 4
                )
                OutlinedTextField(
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text(if (vietnamese) "Người phụ trách" else "Assignee") },
                    singleLine = true
                )
                OutlinedTextField(
                    value = dueDate,
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text(if (vietnamese) "Hạn hoàn thành" else "Due date") },
                    supportingText = {
                        Text(if (vietnamese) "Có thể nhập ngày cụ thể hoặc mô tả như: thứ Sáu, cuối tuần." else "Use a date or a phrase such as Friday or end of week.")
                    },
                    singleLine = true
                )
            }
        ...
        confirmButton = {
            TextButton(
                onClick = {
                    onConfirm(
                        title.trim(),
                        assignee.trim().takeIf(String::isNotBlank),
                        dueDate.trim().takeIf(String::isNotBlank)
                    )
                },
