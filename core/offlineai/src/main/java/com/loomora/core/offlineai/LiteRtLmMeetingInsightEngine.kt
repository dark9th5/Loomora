...
class LiteRtLmMeetingInsightEngine @Inject constructor(
    ...
    override suspend fun analyze(input: MeetingInsightInput): MeetingInsightOutput = withContext(Dispatchers.IO) {
        ...
        val validIds = input.transcriptRevision.segments.map { it.id }.toSet()
    ...
    private fun chunkPrompt(chunk: TranscriptChunk, languageTag: String?): String {
        return buildString {
            appendLine("You are a local meeting analyst. Return one valid JSON object only.")
    ...
    private fun synthesisPrompt(checkpoints: List<InsightChunkCheckpoint>, languageTag: String?): String {
        return buildString {
            appendLine("Merge these chunk JSON results into one deduplicated meeting insight JSON.")
    ...
    private fun schemaInstructions(): String {
        return """
            ...
            Return each array with 0 or 1 best item only. Use only provided segment IDs. Every non-empty item needs evidence. assignee and dueDate are null unless explicit.
        """.trimIndent()
    }
